...
def success(request):
    if request.method == "POST":
        ...
        if(user is not None):#user was found in authentication
            return redirect(request, "success.html", context={'user':user}) #context={'username':username, 'password':password} for testing
        else:
            return redirect(request, "index.html", context={"invalid_user":True})
