...
def index(request):
    if 'invalid_user' in request.session:
        is_invalid = request.session['invalid_user']
        request.session['invalid_user'] = False
    return render(request, "index.html", context={'invalid_user':is_invalid})
